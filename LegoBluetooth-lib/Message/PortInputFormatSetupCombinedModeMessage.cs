    ...
    public class PortInputFormatSetupCombinedModeMessage : CommonMessageHeader
    {
        ...
        public static new PortInputFormatSetupCombinedModeMessage Decode(byte[] data)
        {
            ...
            if (subCommand == PortInputFormatSetupSubCommand.SetModeAndDataSetCombinations)
            {
                ...
                return new PortInputFormatSetupCombinedModeMessage(commonHeader.Length, commonHeader.HubID, portID, subCommand, combinationIndex, modeDataSetCombinations)
                {
            ...
            {
                return new PortInputFormatSetupCombinedModeMessage(commonHeader.Length, commonHeader.HubID, portID, subCommand, 0, null)
                {
                    Message = commonHeader.Message,
                };
            }
        ...
        {
            return $"{base.ToString()}, PortID: {PortID}, SubCommand: {SubCommand}, CombinationIndex: {CombinationIndex}, ModeDataSetCombinations: {BitConverter.ToString(ModeDataSetCombinations)}";
        }
    }
}
